...
for train_id, test_id in outer_cv.split(X1):
    ...
    for antibiotic in ["Piperacillin/Tazobactam", "others", "Cefotaxim", "Meropenem", "Ampicillin/Sulbactam", "Levofloxacin", "Ceftriaxon", "Cefuroxim", "Vancomycin", "Ciprofloxacin", "Piperacillin/Tazobactam-Levofloxacin"]:
        tn, fp, fn, tp = confusion_matrix(np.array(y_test) == antibiotic, np.array(pred) == antibiotic).ravel()        
        sensitivity = tp / (tp + fn)        
        specificity = tn / (tn + fp)        
        precision = tp / (tp + fp)            
        f1 = (2*tp) / (2*tp + fp + fn)
        
        results_all.append([antibiotic, sensitivity, specificity, precision, f1])
...
# replace NaN by zero
for df in range(len(results_outer)):
    results_outer[df] = results_outer[df].fillna(0.000000)
